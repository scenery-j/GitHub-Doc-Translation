...
| 도구 | 최소 버전 | 설명 |
|------|---------|------|
| Node.js | ≥ 20.x | LTS 버전 사용을 권장합니다 |
| npm | ≥ 9.x | 또는 pnpm / yarn 사용 가능 |
...
> 방문 [http://localhost:5173](http://localhost:5173)

### 3. 사전 조건: 백엔드 서비스

프론트엔드 개발 서버는 기본적으로 `/api/*` 요청을 `http://localhost:8080`(으)로 프록시합니다（백엔드 Spring Boot 서비스）。

백엔드 서비스가 실행 중인지 확인해 주세요：
...
| `/settings/quota` | 번역 할당량 및 사용량 세부 내역 | 로그인 필요 |
...
1. 사용자가 'GitHub로 로그인'을 클릭하면 프론트엔드가 직접 `/api/auth/github`로 이동합니다（백엔드가 GitHub로 리다이렉트）。
2. GitHub 인증 후 콜백 `/api/auth/github/callback`。
3. 백엔드 처리 후 프론트엔드 `/dashboard?token=xxx` 또는 `/setup?token=xxx`로 리다이렉트。
4. `App.vue`에서 URL의 `token` 매개변수를 자동으로 추출하여 `localStorage`에 저장。
...
- 백엔드 Spring Boot 서비스가 8080 포트에서 실행 중인지 확인
- vite.config.ts의 프록시 설정 확인

### Q: 로그인 후 /setup으로 리다이렉트되고 /dashboard로 가지 않음

- GitHub App이 아직 설치되지 않았음을 의미합니다. 설치 가이드 단계를 따라 설치를 완료하면 됩니다.

### Q: 번역 설정 페이지의 파일 트리가 비어 있음- 저장소가 플랫폼에 추가되었는지 확인
- GitHub App이 해당 저장소에 대한 접근 권한이 있는지 확인
- 백엔드 인터페이스 GET /api/repos/{id}/tree가 정상적으로 데이터를 반환해야 함

### Q: 백엔드 API 주소를 어떻게 변경하나요?

`vite.config.ts`의 proxy target을 수정하거나, 프로덕션 환경에서 Nginx 역방향 프록시 설정을 통해 변경하세요。

---

> 문제가 있으면 [GitHub Issue](https://github.com/issues)를 제출해 주세요。